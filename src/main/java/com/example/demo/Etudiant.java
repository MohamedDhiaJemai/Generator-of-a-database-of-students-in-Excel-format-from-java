...
public class Etudiant {
	...
	public Etudiant(int idEtudiant, double architecturedesOrdinateurs, double techniquesdeCompilation,
			double rechercheOpérationnelle, double développementMobile, double programmationMultimédias,
			double statistiques, double basesdeDonnéesAvancées, double réseauxInformatiques,
			double conceptionOrientéeObjet, double programmationOrientéeObjet, double conceptiondesBasesdeDonnées,
			double systèmeLogique, double probabilités, double algorithmiqueetStructuresdeDonnées, double blockchain,
			double architectureLogicielle, double systèmesExploitation, double entrepreneuriat, double analyseNumérique,
			double mathPouringénieur, double opt2FouilleDonnées, double opt9BigData, double opt10DeveloppementiOS,
			double opt3MachineLearning, double opt6BusinessIntelligence, double opt4TransactionsElectroniques,
			double opt1CloudComputing, double opt8SécuritéInformatique, double opt5RéseauxIP, double opt16RéseauxGSM,
			double opt12ElectroniqueEmbarquée, double opt13SystèmeRobotique, double opt14ConceptionCartesElectroniques,
			double opt15RéseauCapteurs, double opt7QualitéTest) {
		super();
